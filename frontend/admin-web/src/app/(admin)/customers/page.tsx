 ...
 *
 * This is the Customers module's landing page (`/customers`) - the directory
 * moved to `/customers/directory` so the module opens on this dashboard
 * first, matching Providers' own Onboarding-Overview-first landing (see that
 * page's doc comment).
 *
 * Every status-breakdown tile is a `Link` to `/customers/directory`,
 * pre-filtered to that exact status via the directory's existing `status`
 * query param (same click-through pattern as the Provider Onboarding
 * Overview's tiles). The three registration-cohort tiles (today/7-day/
 * trend-window) compute the exact same UTC calendar-day boundary the backend
 * used - see `windowStartIso` below - so a tile lands on precisely the
 * cohort it summarized; the funnel tiles use the directory's existing (if
...
export default function CustomerAnalyticsPage() {
  ...
  function tileHref(params: Record<string, string>): string {
    return `/customers/directory?${new URLSearchParams(params).toString()}`;
  }
