/**
 * Lazy client-side loader for Firebase Cloud Messaging web push (task 307's
 * customer-side counterpart).
 *
 * Mirrors provider-web's lib/push.ts (and this app's own lib/googleMaps.ts)
 * deliberately: nothing here runs at import time, the Firebase JS SDK loads
 * via a CDN `<script>` tag rather than an npm dependency (no bundler-side
 * Firebase package exists in this repo, and the domain model's
 * DevicePlatform enum only knows Fcm/Apns - it does not know "no push
 * provider configured" as a distinct state, so this has to degrade to null
 * rather than ever calling the registration endpoint with a placeholder
 * token).
 *
 ...
 * before 16.4, most in-app browsers, and even 16.4+ only from an installed
 * home-screen PWA), or the customer declines the permission prompt. Never
 * throws; every failure mode is "no push for this session," which the
 * caller (components/SiteHeader.tsx) treats as routine, not an error to
 * surface.
 */
