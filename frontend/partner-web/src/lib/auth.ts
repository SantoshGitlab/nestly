...
import type { PartnerLoginResponse, PartnerSessionClaims } from "./types";

/**
 * Client-side session storage for the partner portal.
 *
 ...
 *
 * Keys are namespaced "nestly.partner.*" (distinct from admin-web's
 * "nestly.admin.*" and customer-web's "nestly.*") so none of the three apps
 * collide if ever inspected side by side.
 */
const ACCESS_TOKEN_KEY = "nestly.partner.accessToken";
const REFRESH_TOKEN_KEY = "nestly.partner.refreshToken";
const EXPIRES_AT_KEY = "nestly.partner.accessTokenExpiresAt";

/** Notifies subscribed components (layout guard, header) that auth state moved. */
const AUTH_CHANGED_EVENT = "nestly-partner:auth-changed";

function isBrowser(): boolean {
  return typeof window !== "undefined";
}

export function storeSession(session: PartnerLoginResponse): void {
  if (!isBrowser()) return;
 ...
 * uses - partner-api's exact claim names are set by a concurrently-built
 ...
 */
interface RawPartnerTokenPayload {
  sub?: string;
  nameid?: string;
  phone_number?: string;
  mobile?: string;
...
/**
 * Reads the partner's identity off the current access token for "signed in
 * as" display (see components/PartnerHeader.tsx). Returns null when there is
 * no token or it cannot be decoded.
 */
export function getSessionClaims(): PartnerSessionClaims | null {
  const token = getAccessToken();
  if (!token) return null;

  const payload = decodeJwtPayload<RawPartnerTokenPayload>(token);
