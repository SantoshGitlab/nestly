/**
 * Typed fetch wrapper for the Partner API.
...
/**
 * Every partner endpoint is served under the v1 route prefix. Unlike
 * admin-api (a shared project with /api/v1/admin/... prefixed routes),
 * partner-api's entire surface is partner-scoped, so there is no extra
 ...
 * land their underlying entities (see docs/PARTNER.md). Callers use this to
 ...
 * string that partner-api's controller may not use yet.
 */
...
export interface ApiFetchOptions extends RequestInit {
  /** Attaches the stored bearer token. Required by every authenticated partner endpoint. */
...
async function performFetch(path: string, init: ApiFetchOptions | undefined, defaultHeaders: Record<string, string>): Promise<Response> {
  ...
  if (!response.ok) {
    ...
    // login) - clear it so every mounted guard (RequirePartnerAuth) reacts
    // to the auth-changed event and sends the partner back to /login. An
