...
import type { PartnerOnboardingStatus, PartnerProfile, PartnerStatus } from "@/lib/types";
...
/**
 * The onboarding funnel in order (docs/PARTNER.md). Rendered as a progress
 * track rather than a bare label: "KycSubmitted" tells a partner nothing about
 * how much is left, and how much is left is the only reason they look.
 */
const ONBOARDING_STEPS: readonly { value: PartnerOnboardingStatus; label: string }[] = [
...
export function ProfileDetailsSection() {
  ...
  const query = useQuery({ queryKey: ["partner-profile"], queryFn: getProfile });
  ...
  const mutation = useMutation({
    ...
    onSuccess: (profile) => {
      queryClient.setQueryData(["partner-profile"], profile);
      setIsEditing(false);
...
function initialsOf(profile: PartnerProfile): string {
...
/**
 * Where the partner is in onboarding, as a track of steps. Steps at or before
 ...
 */
function OnboardingProgress({ status }: { status: PartnerOnboardingStatus }) {
