 ...
 * partner does in this app and it is time-boxed by `responseDeadline`, so it
...
export default function JobDetailPage() {
  ...
  const verificationQuery = useQuery({
    queryKey: ["partner-job-completion-verification", jobId],
    queryFn: () => getCompletionVerification(jobId),
  });

  const invalidate = () => {
    queryClient.invalidateQueries({ queryKey: ["partner-job", jobId] });
    queryClient.invalidateQueries({ queryKey: ["partner-job-completion-verification", jobId] });
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      invalidate();
      toast("info", "Job declined. It will be offered to another partner.");
    },
  });
  ...
  return (
    <div>
        ...
        <section className="mb-6 overflow-hidden rounded-2xl border border-warning/40 bg-surface shadow-sm">
          <div className="border-b border-warning/25 bg-warning-soft px-5 py-3">
            ...
            <p className="mt-0.5 text-xs text-warning/90">
              If you don&apos;t respond in time, this job goes to another partner.
            </p>
          </div>
      ...
      <Modal
        ...
        description="It will be offered to another partner and you won't be able to take it back."
        size="sm"
        footer={
          <>
            <Button
