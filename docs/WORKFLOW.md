...
provider/provider module, [tasks.csv](tasks.csv) for what's built vs.
...
and domain model call the same role "Provider" (see PROVIDER.md) — a deliberate
rename made when the module was scoped, currently deferred to Phase 8. This
document uses **Provider**, and calls out the sketch's term in parentheses the
...
flowchart TD
    ...
    C -->|Provider, aka Provider| E[Provider workflow]
```
...
## 3. Provider Workflow (Provider / Service Provider)
