...
one `Completed` booking, and provider Ravi K's `Active` status all still
...
proof, full status timeline, and (admin-side) provider assignment panel
those rows' notes describe are actually visible rather than cut off below
the fold.

`provider-web`'s `login` screen was recaptured in both themes the same way.
...
them requires a live provider session, which requires completing the mobile
...
provider-api to pick up that change was also declined). Those five images
...
temporary server-side log, restart provider-api, capture, and revert.
