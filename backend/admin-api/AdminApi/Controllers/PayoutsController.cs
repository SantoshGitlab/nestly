...
/// <summary>
/// Admin partner payout batches (PARTNER.md Financial Domain, API surface
/// "run payout batch, list payouts"; task 148). OPEN DECISIONS #3: v1 is
/// manual bank transfer - a batch is created from the partner's earning
/// ledger for a period, then an admin walks its status Pending -&gt;
/// Processing -&gt; Paid/Failed by hand; there is no gateway integration.
/// Gated "payout.read"/"payout.write" (PARTNER.md RBAC ADDITIONS).
/// </summary>
[ApiController]
[ApiVersion(1)]
...
public class PayoutsController : ControllerBase
{
    ...
    private readonly IValidator<CreatePartnerPayoutRequest> _createValidator;
    private readonly IValidator<UpdatePartnerPayoutStatusRequest> _updateStatusValidator;

    public PayoutsController(
        IPartnerPayoutService payoutService,
        IValidator<CreatePartnerPayoutRequest> createValidator,
        IValidator<UpdatePartnerPayoutStatusRequest> updateStatusValidator)
    {
    ...
    /// <summary>Search/filter payouts by partner and/or status.</summary>
    [HttpGet]
    [Authorize(Policy = ReadPolicy)]
    [ProducesResponseType(typeof(PartnerPayoutSearchResponse), StatusCodes.Status200OK)]
    public async Task<IActionResult> Search(
        [FromQuery] Guid? partnerId,
        [FromQuery] PartnerPayoutStatus? status,
        [FromQuery] int page = 1,
        [FromQuery] int pageSize = 20)
    {
        var result = await _payoutService.SearchAsync(partnerId, status, page, pageSize);
    ...
    /// <summary>Runs a payout batch for a partner over a period, summing their earning ledger (task 148).</summary>
    [HttpPost("partners/{partnerId:guid}")]
    [Authorize(Policy = WritePolicy)]
    [ProducesResponseType(typeof(PartnerPayoutResponse), StatusCodes.Status201Created)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status422UnprocessableEntity)]
    public async Task<IActionResult> CreateBatch(Guid partnerId, [FromBody] CreatePartnerPayoutRequest request)
    {
        ...
        var result = await _payoutService.CreateBatchAsync(partnerId, request);
        return result.IsSuccess ? Ok(result.Value) : result.ToProblemResult();
    }
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status422UnprocessableEntity)]
    public async Task<IActionResult> UpdateStatus(Guid payoutId, [FromBody] UpdatePartnerPayoutStatusRequest request)
    {
