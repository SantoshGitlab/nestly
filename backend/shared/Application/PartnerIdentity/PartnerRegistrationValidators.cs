...
public class RequestPartnerRegistrationOtpRequestValidator : AbstractValidator<RequestPartnerRegistrationOtpRequest>
{
    public RequestPartnerRegistrationOtpRequestValidator()
    {
        RuleFor(x => x.Mobile)
...
public class RegisterPartnerRequestValidator : AbstractValidator<RegisterPartnerRequest>
{
    public RegisterPartnerRequestValidator()
    {
        RuleFor(x => x.Mobile)
            .NotEmpty().WithMessage("Mobile number is required")
            .Matches(@"^\+?[1-9]\d{7,14}$").WithMessage("Mobile number must be a valid phone number");
