...
/// customer- and partner-side counterpart to Referral's
/// <c>ReferralQualifyingBookingHandler</c>/<c>IReferralRewardService</c>,
/// reusing the same wallet/partner-earning-ledger credit primitives rather
...
public interface INestlyCoinsService
{
    ...
    /// Completed orders the subject (customer or partner) already has
    ...
    /// <summary>
    /// Credits the booking's assigned partner's earning ledger if the
    /// Partner-audience program is active and <see cref="EvaluateQualifyingOrder"/>
    /// passes. No-op when there is no assigned partner, no Partner config,
    /// it is inactive, or the order doesn't qualify.
    /// </summary>
    Task CreditPartnerCoinsAsync(Booking booking);

    /// <summary>
    /// Reverses any Nestly Coins credit (customer and/or partner side)
