...
/// <summary>
/// A single OTP challenge for a partner (PARTNER.md "partner_otp ... mirrors
...
/// module stays independent of Customer per PARTNER.md's SCOPE BOUNDARY: a
/// person who is both a customer and a partner with the same mobile number
/// must not have one role's OTP/lockout state affect the other's.
/// </summary>
public class PartnerOtp : Entity<Guid>
{
    ...
    public PartnerOtp(Guid id, Guid? partnerId, string target, OtpPurpose purpose, string codeHash, DateTime expiresAt)
        : base(id)
    {
        PartnerId = partnerId;
