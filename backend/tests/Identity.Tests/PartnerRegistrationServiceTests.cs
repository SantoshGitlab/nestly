...
/// <summary>
/// Partner registration (task 146a), structurally mirroring how
...
/// </summary>
public class PartnerRegistrationServiceTests : IDisposable
{
    ...
    public PartnerRegistrationServiceTests()
    {
        _otpService
    ...
    private PartnerRegistrationService CreateService(NestlyDbContext context) =>
        new(new PartnerRepository(context), new PartnerAuthIdentityRepository(context), _otpService.Object);

    private static RegisterPartnerRequest ValidRequest(string mobile = Mobile) => new(
        mobile, OtpCode, "Ravi Kumar", "Ravi's Repairs", "ravi@example.com", ConsentAccepted: true);

    [Fact]
    public async Task RegisterAsync_creates_a_partner_pending_verification_with_a_registered_onboarding_status()
    {
        ...
        result.Value.OnboardingStatus.Should().Be(nameof(PartnerOnboardingStatus.Registered));

        var stored = await context.Set<Partner>().SingleAsync();
        stored.PartnerType.Should().Be(PartnerType.Individual);
        stored.Phone.Should().Be(Mobile);
    }

    [Fact]
    public async Task RegisterAsync_creates_a_mobile_otp_auth_identity_for_the_new_partner()
    {
        await using var context = _database.CreateContext();
        var result = await CreateService(context).RegisterAsync(ValidRequest());

        var identity = await context.Set<PartnerAuthIdentity>().SingleAsync();
        identity.PartnerId.Should().Be(result.Value.Id);
    ...
    public async Task RegisterAsync_rejects_a_mobile_number_that_is_already_registered()
    {
        ...
        second.Error.Code.Should().Be("PartnerRegistration.MobileAlreadyRegistered");
    }
    ...
    public async Task RegisterAsync_rejects_registration_without_consent()
    {
        ...
        result.Error.Code.Should().Be("PartnerRegistration.ConsentRequired");
        (await context.Set<Partner>().CountAsync()).Should().Be(0);
    }
    ...
    public async Task RegisterAsync_propagates_an_otp_validation_failure()
    {
        _otpService
            ...
            .ReturnsAsync(Result.Failure(Error.Validation("PartnerOtp.Incorrect", "The OTP code is incorrect.")));
    ...
    [Fact]
    public async Task RequestOtpAsync_refuses_a_mobile_number_already_registered_as_a_partner()
    {
        await using var context = _database.CreateContext();
        var service = CreateService(context);
        await service.RegisterAsync(ValidRequest());

        var result = await service.RequestOtpAsync(new RequestPartnerRegistrationOtpRequest(Mobile));

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("PartnerRegistration.MobileAlreadyRegistered");
    }
