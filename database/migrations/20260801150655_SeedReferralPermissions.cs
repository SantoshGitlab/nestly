...
{
    /// <summary>
    /// Task 173: seeds the new Referral permission module REFERRAL.md's RBAC
    /// ADDITIONS calls for, added to <see cref="AdminModules.All"/> alongside
    /// this migration. Same incremental-seed shape as
    /// 20260731233627_SeedPartnerPayoutPermissions.cs - only inserts the new
    /// module's rows; every other module's rows already exist from 96a
    /// (AddAdminPermissionMatrix) or the partner/payout follow-up.
    /// </summary>
    public partial class SeedReferralPermissions : Migration
    {
        // Distinct fixed timestamp from 96a's and the partner/payout
        // migration's, same "reference data, not a real event" rationale.
        private static readonly DateTime SeedTimestamp = new(2026, 8, 1, 15, 6, 0, DateTimeKind.Utc);

        private static readonly string[] NewModules = [AdminModules.Referral];
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                ...
                "(SELECT id FROM admin_permission WHERE module = 'referral');");
            migrationBuilder.Sql("DELETE FROM admin_permission WHERE module = 'referral';");
        }
    }
}
