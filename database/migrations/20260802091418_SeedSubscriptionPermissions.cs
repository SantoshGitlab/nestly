...
{
    /// <summary>
    /// Task 180: seeds the new Subscription permission module
    /// (PRODUCT-ENHANCEMENTS.md #1, RBAC ADDITIONS "View / Configure"),
    /// added to <see cref="AdminModules.All"/> alongside this migration.
    /// Same incremental-seed shape as 20260801210314_SeedChatPermissions.cs -
    /// only inserts the new module's rows; every other module's rows
    /// already exist from 96a (AddAdminPermissionMatrix) or a later
    /// module's own seed migration.
    /// </summary>
    public partial class SeedSubscriptionPermissions : Migration
    {
        // Distinct fixed timestamp from every earlier seed migration's, same
        // "reference data, not a real event" rationale.
        private static readonly DateTime SeedTimestamp = new(2026, 8, 2, 9, 14, 0, DateTimeKind.Utc);

        private static readonly string[] NewModules = [AdminModules.Subscription];
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                ...
                "(SELECT id FROM admin_permission WHERE module = 'subscription');");
            migrationBuilder.Sql("DELETE FROM admin_permission WHERE module = 'subscription';");
        }
    }
}
