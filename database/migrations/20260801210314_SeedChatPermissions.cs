...
{
    /// <summary>
    /// Task 193/194: seeds the new Chat permission module (PRODUCT-ENHANCEMENTS.md
    /// IN-APP CHAT, RBAC ADDITIONS), added to <see cref="AdminModules.All"/>
    /// alongside this migration. Same incremental-seed shape as
    /// 20260801150655_SeedReferralPermissions.cs - only inserts the new
    /// module's rows; every other module's rows already exist from 96a
    /// (AddAdminPermissionMatrix) or a later module's own seed migration.
    /// </summary>
    public partial class SeedChatPermissions : Migration
    {
        // Distinct fixed timestamp from every earlier seed migration's, same
        // "reference data, not a real event" rationale.
        private static readonly DateTime SeedTimestamp = new(2026, 8, 1, 21, 3, 0, DateTimeKind.Utc);

        private static readonly string[] NewModules = [AdminModules.Chat];
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                ...
                "(SELECT id FROM admin_permission WHERE module = 'chat');");
            migrationBuilder.Sql("DELETE FROM admin_permission WHERE module = 'chat';");
        }
    }
}
